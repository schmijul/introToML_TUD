...
def process_batch(model, batch_x, batch_y, current_lr, gradient_clip):
    loss = backpropagation(model, batch_x, batch_y, current_lr)
    if gradient_clip is not None:
        grad_w2 = np.matmul(model.linear1.outputs.T, d_loss)
        grad_b2 = np.sum(d_loss, axis=0)
        d_sigmoid = sigmoid(model.linear1.outputs) * (1 - sigmoid(model.linear1.outputs))
...
def train(model, x_train, y_train, num_epochs, init_lr=1, target_loss=None, scale_data=False, decay_lr=None, decay_interval=None, gradient_clip=None, verbose=False, batchsize=100):
    current_lr = init_lr
    best_loss = np.inf
    ...
    for current_epoch in range(num_epochs):
        ...
        for i in range(0, len(x_train), batchsize):
            ...
            result = pool.apply_async(process_batch, args=(model, batch_x, batch_y, current_lr, gradient_clip))
            batch_losses.append(result)

        # Wait for all batch processes to finish
        batch_losses = [res.get() for res in batch_losses]
...
def main():
    ...
    best_weights_and_biases, train_loss = train(model, x_train, y_train, num_epochs=num_epochs, init_lr=init_lr, target_loss=target_loss, scale_data=scale_data, verbose=verbose, batchsize=batchsize)
